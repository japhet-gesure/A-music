...
export function MusicPlayer() {
  ...
  return (
    <div id="player-outer" className="h-full relative flex flex-col justify-center px-8">
      {/* YouTube / Cloud Player Component */}
        ...
        <>
          <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-6">
                <motion.button 
                  ...
                  className={cn(
                    ...
                    shuffleMode 
                      ? "text-purple-400 bg-purple-500/10 border-purple-500/30 shadow-[inset_0_0_20px_rgba(168,85,247,0.2)]" 
                ...
                <div className="flex items-center gap-5 bg-white/[0.03] rounded-[1.5rem] px-6 py-2 border border-white/10 backdrop-blur-3xl shadow-[0_20px_50px_rgba(0,0,0,0.5)] relative group/controls">
                  ...
                  <motion.button 
                    ...
                    className={cn(
                      ...
                      hasPrev ? "text-white/70 hover:text-white hover:bg-white/10" : "text-white/5 cursor-not-allowed"
                    )}
                    title="Previous"
                  >
                  ...
                  <motion.button 
                    ...
                    className={cn(
                      "transition-all p-2 rounded-full relative z-10", 
                      (hasNext || repeatMode === "all") ? "text-white/70 hover:text-white hover:bg-white/10" : "text-white/5 cursor-not-allowed"
                    )}
                    title="Next"
                  >
                ...
                <motion.button 
                  ...
                  className={cn(
                    ...
                    repeatMode !== "off" 
                      ? "text-indigo-400 bg-indigo-500/10 border-indigo-500/30 shadow-[inset_0_0_20px_rgba(99,102,241,0.2)]" 
                      : "text-white/20 border-transparent hover:text-white/40 hover:bg-white/5"
                  )}
                ...
                >
                  <Repeat size={16} />
                  {repeatMode === "one" && (
                    <span className="absolute -top-1.5 -right-1.5 text-[9px] font-black bg-indigo-500 text-white w-5 h-5 rounded-full flex items-center justify-center shadow-2xl border-2 border-indigo-400 z-20">1</span>
                  )}
                </motion.button>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6 w-1/4 justify-end">
              <div className="flex items-center p-1.5 bg-white/[0.03] rounded-2xl border border-white/10 backdrop-blur-3xl shadow-xl">
                <div className="flex items-center gap-1.5">
                  <div className="relative">
                    <button 
                      ...
                      className={cn(
                        ...
                        showEQ ? "bg-purple-500 text-white" : "text-white/30 hover:text-white hover:bg-white/10"
                      )}
                      title="Equalizer"
                    >
                  ...
                  <button 
                    ...
                    className={cn(
                      "p-2.5 rounded-xl transition-all", 
                      showLyrics ? "bg-pink-500 text-white" : "text-white/30 hover:text-white hover:bg-white/10"
                    )}
                    title="Lyrics"
                  >
                    <Mic2 size={16} />
                  ...
                  <button 
                    ...
                    className={cn(
                      "p-2.5 rounded-xl transition-all", 
                      showNowPlaying ? "bg-blue-600 text-white" : "text-white/30 hover:text-white hover:bg-white/10"
                    )}
                    title="Queue & History"
                  >
                    <LayoutList size={20} />
              ...
              <div className="flex items-center gap-4 group px-4 py-3 bg-white/[0.02] rounded-2xl border border-transparent hover:border-white/10 transition-all shadow-lg">
                <Volume2 size={18} className="text-white/30 group-hover:text-white transition-all" />
                <div className="w-28 h-2 bg-white/5 rounded-full relative cursor-pointer overflow-hidden border border-white/5">
                  ...
                  />
                  <div className="absolute top-0 left-0 h-full bg-white group-hover:bg-purple-600 transition-all" style={{ width: `${volume * 100}%` }} />
                </div>
              </div>
            </div>
          </div>
          ...
          <AnimatePresence>
            {showNowPlaying && (
              ...
              >
                 <button onClick={() => setShowNowPlaying(false)} className="absolute top-8 left-8 text-white/30 hover:text-white transition-colors"><X size={24} /></button>
                 <div className="flex bg-white/5 rounded-full p-1 mb-12">
                   <button onClick={() => setNowPlayingTab("playing")} className={cn("flex-1 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all", nowPlayingTab === "playing" ? "bg-white text-black" : "text-white/40 hover:text-white")}>Now Playing</button>
                   <button onClick={() => setNowPlayingTab("queue")} className={cn("flex-1 px-4 py-2 rounded-full text-[10px] font-black uppercase tracking-widest transition-all", nowPlayingTab === "queue" ? "bg-white text-black" : "text-white/40 hover:text-white")}>Queue ({queue.length})</button>
                 </div>
                 <AnimatePresence mode="wait">
                   {nowPlayingTab === "playing" ? (
                     <motion.div key="playing" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex-1 flex flex-col justify-center gap-12">
                        <div className="relative aspect-square w-full rounded-[2.5rem] overflow-hidden shadow-2xl border border-white/5"><img src={currentSong.thumbnail || undefined} alt={currentSong.title} className="w-full h-full object-cover" /></div>
                        <div className="space-y-6">
                           <div className="space-y-2"><h2 className="text-4xl font-black italic tracking-tighter text-white leading-tight">{currentSong.title}</h2><p className="text-xl text-purple-400 font-bold uppercase tracking-widest">{currentSong.artist}</p></div>
                           <div className="flex items-center gap-2"><div className="px-3 py-1 bg-white/5 rounded-full border border-white/10"><span className="text-[10px] font-black text-white/40 uppercase tracking-widest">{currentSong.source} SIGNAL</span></div></div>
                        </div>
                     </motion.div>
                   ) : (
                     <motion.div key="queue" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="flex-1 flex flex-col overflow-hidden">
                       <Reorder.Group axis="y" values={queue} onReorder={setQueue} className="flex-1 overflow-y-auto space-y-2 pr-2 scroll-hide">
                         {queue.map((song, index) => (
                           <Reorder.Item value={song} key={song.id} className={cn("group flex items-center gap-4 p-3 rounded-2xl border transition-all relative cursor-grab active:cursor-grabbing", index === currentIndex ? "bg-white/5 border-purple-500/50" : "bg-white/[0.02] border-white/5 hover:bg-white/[0.05]")}>
                             <div className="relative w-12 h-12 shrink-0 rounded-lg overflow-hidden"><img src={song.thumbnail || undefined} className="w-full h-full object-cover" /></div>
                             <div className="flex-1 min-w-0 pr-2"><h4 className={cn("text-sm font-bold truncate", index === currentIndex ? "text-purple-400" : "text-white")}>{song.title}</h4><p className="text-[10px] text-white/40 uppercase font-medium tracking-wider truncate">{song.artist}</p></div>
                             <div className="opacity-0 group-hover:opacity-100 transition-opacity"><button onClick={(e) => { e.stopPropagation(); removeFromQueue(song.id); }} className="p-2 text-zinc-600 hover:text-red-500 transition-colors"><Trash2 size={16} /></button></div>
                             {index !== currentIndex && <button onClick={() => setSong(song)} className="absolute inset-0 z-0 pr-12" />}
                           </Reorder.Item>
                         ))}
                       </Reorder.Group>
                     </motion.div>
            ...
            {showLyrics && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bottom-24 bg-[#0A0A0C] z-40 p-12 lg:p-24 flex flex-col items-center overflow-y-auto">
                 <button onClick={() => setShowLyrics(false)} className="absolute top-8 right-8 w-14 h-14 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-white/10 transition-all z-50"><X size={24} /></button>
                 <div className="max-w-4xl w-full pt-10 space-y-16 pb-40 z-10 h-full overflow-y-auto scroll-hide">
                    <div className="flex flex-col md:flex-row items-center gap-12 text-center md:text-left"><img src={currentSong.thumbnail || undefined} className="w-48 h-48 rounded-3xl shadow-2xl" /><div><h1 className="text-4xl font-black mb-4 tracking-tighter italic">{currentSong.title}</h1><p className="text-xl text-purple-400 font-bold uppercase tracking-widest">{currentSong.artist}</p></div></div>
                    <div className="space-y-10 text-5xl font-black italic tracking-tighter">
                       {currentSong.lyrics && currentSong.lyrics.length > 0 ? currentSong.lyrics.map((line, i) => {
                         const isActive = line.time > 0 && currentTime >= line.time && (i === currentSong.lyrics!.length - 1 || currentTime < currentSong.lyrics![i+1].time);
                         return <p key={i} className={cn("transition-all duration-500 cursor-pointer origin-left", isActive ? "text-white" : "text-white/5 hover:text-white/20")} onClick={() => { if (line.time > 0) { if (currentSong?.source === "local" && audioRef.current) audioRef.current.currentTime = line.time; setProgress(line.time); } }}>{line.text}</p>;
                       }) : <div className="py-20 flex flex-col items-center"><p className="text-white/10 mb-12 text-7xl uppercase">No Lyrics Found</p><button onClick={() => setShowLyricsEditor(true)} className="px-12 py-5 bg-white text-black rounded-full font-black uppercase italic text-sm">Edit Lyrics</button></div>}
                    </div>
                 </div>
                 <AnimatePresence>{showLyricsEditor && <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="fixed inset-y-0 right-0 w-full lg:w-1/2 z-[60]"><LyricsEditor onClose={() => setShowLyricsEditor(false)} /></motion.div>}</AnimatePresence>
              </motion.div>
            )}
          </AnimatePresence>
